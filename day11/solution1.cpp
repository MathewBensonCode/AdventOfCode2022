...
const auto get_operation = [](const auto &operation_string) {
  auto operands = operation_string | std::views::split(space_delimiter) | std::views::drop(1);
...
const auto get_monkeys = [](const auto &section) {
  ...
  auto monkey_items = *current_items | std::views::split(item_delimiter) | std::views::transform(get_num_from_string);

  auto operation_line = std::next(items_line);

  auto operation = *operation_line | std::views::split(operation_delimiter) | std::views::drop(1) | std::views::transform(get_operation);

  auto divide_by_line = std::next(operation_line);
  const auto number_of_spaces_to_divide_num{ 5 };
  auto divide_by_num = *divide_by_line | std::views::split(space_delimiter) | std::views::drop(number_of_spaces_to_divide_num) | std::views::transform(get_num_from_string);

  auto true_line = std::next(divide_by_line);
  const auto number_of_spaces_to_true_num{ 9 };
  auto true_num = *true_line | std::views::split(space_delimiter) | std::views::drop(number_of_spaces_to_true_num) | std::views::transform(get_num_from_string);

  auto false_line = std::next(true_line);
  const auto number_of_spaces_to_false_num{ 9 };
  auto false_num = *false_line | std::views::split(space_delimiter) | std::views::drop(number_of_spaces_to_false_num) | std::views::transform(get_num_from_string);
...
int main()
{
    try {
      ...
      auto monkeys = inputdatastring | std::views::split(sectiondelimiter) | std::views::transform(get_monkeys);
