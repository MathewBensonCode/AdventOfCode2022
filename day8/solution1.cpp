...
int main() {
  ...
  std::array<unsigned long, counter_length> visible_tree_count_store{};

  auto get_index_for_array = [](unsigned long row,
                                unsigned long col) -> unsigned long {
    return col + (row * array_width);
  };

  auto print_found = [&](const std::string &name) {
    std::cout << name << '\n';
    for (unsigned long row_index{0}; row_index < array_width; ++row_index) {
  ...
  auto count_trees_top_to_bottom = [&]() {
    for (unsigned long col_index{0}; col_index < array_width; ++col_index) {
      char highest_tree{};
      for (unsigned long row_index{0}; row_index < array_width; ++row_index) {
        const char current_tree = datastore.at(row_index).at(col_index);
  ...
  auto count_trees_right_to_left = [&]() {
    for (auto row_index{0}; row_index < array_width; ++row_index) {
      ...
      for (auto col_index{array_width - 1}; col_index >= 0; --col_index) {
        auto rowval = static_cast<unsigned long>(row_index);
        auto colval = static_cast<unsigned long>(col_index);
