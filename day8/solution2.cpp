...
int main()
{
    try {
      ...
      auto get_left_to_right_view = [&](std::size_t row,
                                      std::size_t col) -> auto {
        auto current_view = std::string_view{ inputdata } | joint_view | std::views::drop((row * array_width) + col) | std::views::take(array_width - col);
      ...
      auto get_right_to_left_view = [&](std::size_t row,
        ...
        auto current_view = *myview | std::views::reverse | std::views::drop(array_width - col - 1);
      ...
      auto get_bottom_to_top_view = [&](std::size_t row,
                                      std::size_t col) -> auto {
        std::vector<char> results{};
        auto rowviews = std::string_view{ inputdata } | reverse_split_view | std::views::drop(array_width - (row + 1));
